...
import {getApartamentoDePersona,paquetesApi,reservasApi,visitasApi,autorizarVisita,cancelarReserva,zonasApi,getTiposVisita,getTiposDocumento} from "../../api";
...
const crearPayloadVisita = (form, apartamentoId) => {
 ...
 return {tipoVisita:{id:tipoVisitaId},tipoDocumento:{id:tipoDocumentoId},nombreVisitante,documentoVisitante,motivoVisita:motivoVisita||null,fechaIngreso,fechaSalida:null,estadoVisita:{id:1},apartamento:{id:apartamentoId},origenRegistro:"RESIDENTE"};
};
...
export default function ResidenteModulePage({type}){
 ...
 const filtrosReporte=[
  ...
  filters.estado&&`Estado: ${filters.estado}`,
  filters.tipo&&`${filterLabel}: ${filterOptions.find(x=>String(x.value)===filters.tipo)?.label||filters.tipo}`,
 ...
 {type==="visitas"&&<table className="module-table"><thead><tr><th>Visitante</th><th>Tipo</th><th>Motivo</th><th>Ingreso</th><th>Salida</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{filteredItems.length?filteredItems.map(x=><tr key={x.id}><td>{x.nombreVisitante}</td><td>{x.tipoVisita?.nombre}</td><td>{x.motivoVisita||"—"}</td><td>{x.fechaIngreso?.replace("T"," ").slice(0,16)}</td><td>{x.fechaSalida?.replace("T"," ").slice(0,16)||"—"}</td><td><span className="badge badge-info">{x.estadoVisita?.nombre}</span></td><td>{x.estadoVisita?.nombre?.toLowerCase()==="pendiente"&&<button className="small-btn" onClick={()=>autorizar(x.id)}>Autorizar</button>}</td></tr>):<tr><td colSpan="7" className="empty-row">No tienes visitas.</td></tr>}</tbody></table>}
 </div>
