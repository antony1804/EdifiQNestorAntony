...
import { sanitizeUsername, validateCredentials } from "../utils/validation";
...
function RegistroPage() {
  ...
  const handleChange = (e) => setForm({ ...form, [e.target.name]: e.target.name === "username" ? sanitizeUsername(e.target.value) : e.target.value });

  const handleSubmit = async (e) => {
    e.preventDefault();
    setMensaje("");
    const validacion = validateCredentials(form);
    if (validacion) {
      setMensaje(validacion);
      return;
    }
    if (!form.idPersona || !form.idRol) {
      setMensaje("Selecciona la persona y el rol.");
      return;
    }
    const payload = {
