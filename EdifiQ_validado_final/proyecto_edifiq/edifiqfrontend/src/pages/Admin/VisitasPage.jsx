...
export default function VisitasPage() {
	...
	const submit = async (e) => {
		...
		if (!form.tipoVisitaId || !form.tipoDocumentoId || !form.apartamentoId || !form.estadoId) return setError("Completa todos los campos obligatorios.");
		if (!/^[\\p{L}]+(?:[ '\\-][\\p{L}]+)*$/u.test(form.nombreVisitante.trim())) return setError("El nombre solo puede contener letras, espacios, apóstrofes y guiones.");
		if (!/^[A-Za-z0-9.-]{3,30}$/.test(form.documentoVisitante)) return setError("El documento contiene caracteres no permitidos.");
		if (!form.fechaIngreso) return setError("La fecha de ingreso es obligatoria.");
		if (form.fechaSalida && form.fechaSalida < form.fechaIngreso) return setError("La salida no puede ser anterior al ingreso.");
						...
						<div className="form-grid">
							...
							<div className="form-group">
								...
								<input
									...
									onChange={(e) =>
										setForm({ ...form, documentoVisitante: sanitizeDocument(e.target.value, 30) })
									}
								/>
							</div>
							...
							<div className="form-group">
								...
								<input
									...
									onChange={(e) =>
										setForm({ ...form, nombreVisitante: sanitizeName(e.target.value) })
									}
								/>
							</div>
							...
							<div className="form-group">
								...
								<input
									...
									onChange={(e) =>
										setForm({ ...form, motivoVisita: sanitizeText(e.target.value, 150) })
									}
								/>
							</div>
