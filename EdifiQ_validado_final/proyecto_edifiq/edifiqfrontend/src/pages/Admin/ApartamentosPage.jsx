...
export default function ApartamentosPage() {
	...
	const submit = async (e) => {
		...
		if (!form.numeroApartamento.trim()) return setError("El número del apartamento es obligatorio.");
		if (form.piso === "" || Number(form.piso) < 0 || Number(form.piso) > 200) return setError("Ingresa un piso válido entre 0 y 200.");
		if (!form.torreId) return setError("Selecciona una torre.");
			...
			<div className="form-grid">
				<div className="form-group">
					...
					<input
						...
						onChange={(e) =>
							setForm({ ...form, numeroApartamento: sanitizeText(e.target.value.toUpperCase(), 10).replace(/[^A-Z0-9-]/g, "") })
						}
					/>
				</div>
				<div className="form-group">
					...
					<input
						...
						min="0"
						value={form.piso}
						onChange={(e) => setForm({ ...form, piso: e.target.value.replace(/\D/g, "").slice(0, 3) })}
					/>
				</div>
				<div className="form-group">
					<label>Torre</label>
