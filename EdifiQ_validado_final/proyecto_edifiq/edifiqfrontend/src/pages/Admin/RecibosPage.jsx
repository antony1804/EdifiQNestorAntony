...
import { sanitizePeriod, validatePositiveNumber } from "../../utils/validation";
...
export default function RecibosPage() {
	...
	const submit = async (e) => {
		...
		if (!form.tipoServicioId || !form.apartamentoId || !form.estadoId) return setError("Completa todos los campos obligatorios.");
		if (form.periodo.trim().length < 4) return setError("El periodo debe tener al menos 4 caracteres.");
		const valueError = validatePositiveNumber(form.valor, "El valor");
		if (valueError) return setError(valueError);
		if (!form.fechaEmision || !form.fechaVencimiento) return setError("Las fechas son obligatorias.");
		if (form.fechaVencimiento < form.fechaEmision) return setError("La fecha de vencimiento no puede ser anterior a la emisión.");
						...
						<div className="form-grid">
							...
							<div className="form-group">
								...
								<input
									...
									value={form.periodo}
									onChange={(e) => setForm({ ...form, periodo: sanitizePeriod(e.target.value) })}
								/>
							</div>
