...
export default function PaquetesPage() {
	...
	const submit = async (e) => {
		...
		if (form.descripcion.trim().length < 3) return setError("La descripción debe tener al menos 3 caracteres.");
		if (form.remitente.trim().length < 2) return setError("El remitente debe tener al menos 2 caracteres.");
		if (!form.apartamentoId) return setError("Selecciona un apartamento.");
		if (!form.estadoId) return setError("Selecciona un estado.");
		if (!form.fechaRecepcion) return setError("La fecha de recepción es obligatoria.");
		if (form.fechaEntrega && form.fechaEntrega < form.fechaRecepcion) return setError("La entrega no puede ser anterior a la recepción.");
						...
						<div className="form-grid">
							<div className="form-group full">
								...
								<input
									...
									onChange={(e) =>
										setForm({ ...form, descripcion: sanitizeText(e.target.value, 200) })
									}
								/>
							</div>
							...
							<div className="form-group">
								...
								<input
									...
									onChange={(e) =>
										setForm({ ...form, remitente: sanitizeText(e.target.value, 100) })
									}
								/>
							</div>
