...
public class ReciboService {
	...
	public Recibo guardar(Recibo r) {
		r.setPeriodo(r.getPeriodo().trim().replaceAll("\\s+", " "));
		validar(r);
		if (repo.existsByApartamento_IdAndTipoServicio_IdAndPeriodoIgnoreCase(r.getApartamento().getId(), r.getTipoServicio().getId(), r.getPeriodo())) {
			throw new IllegalArgumentException("Ya existe un recibo para ese apartamento, servicio y periodo.");
		}
	...
	public Recibo actualizar(Long id, Recibo d) {
		...
		d.setPeriodo(d.getPeriodo().trim().replaceAll("\\s+", " "));
		validar(d);
		if (repo.existsByApartamento_IdAndTipoServicio_IdAndPeriodoIgnoreCaseAndIdNot(d.getApartamento().getId(), d.getTipoServicio().getId(), d.getPeriodo(), id)) {
			throw new IllegalArgumentException("Ya existe un recibo para ese apartamento, servicio y periodo.");
		}
