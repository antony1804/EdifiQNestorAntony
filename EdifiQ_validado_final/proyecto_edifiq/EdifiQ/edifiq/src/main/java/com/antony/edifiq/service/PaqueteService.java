...
public class PaqueteService {
	...
	public Paquete guardar(Paquete p) {
		p.setDescripcion(p.getDescripcion().trim().replaceAll("\\s+", " "));
		p.setRemitente(p.getRemitente().trim().replaceAll("\\s+", " "));
		validar(p);
	...
	public Paquete actualizar(Long id, Paquete d) {
		...
		d.setDescripcion(d.getDescripcion().trim().replaceAll("\\s+", " "));
		d.setRemitente(d.getRemitente().trim().replaceAll("\\s+", " "));
		validar(d);
